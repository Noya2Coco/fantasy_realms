...
export function createSceneAxis(scene, size) {
    ...
    const xText = makeTextPlane(scene, "X", "red", 256, 256, 200);
    ...
    const yText = makeTextPlane(scene, "Y", "green", 256, 256, 200);
    ...
    const zText = makeTextPlane(scene, "Z", "blue", 256, 256, 200);
...
export function createMeshAxis(mesh, scene, size) {
    ...
    axisX.color = Color3.Red();
    axisX.parent = mesh;
    const xText = makeTextPlane(scene, "X", "red", 128, 128, 60);
    xText.position = new Vector3(size * 1.1, 0, 0);
    xText.parent = mesh;
    ...
    axisY.color = Color3.Green();
    axisY.parent = mesh;
    const yText = makeTextPlane(scene, "Y", "green", 128, 128, 60);
    yText.position = new Vector3(0, size * 1.1, 0);
    yText.parent = mesh;
    ...
    axisZ.color = Color3.Blue();
    axisZ.parent = mesh;
    const zText = makeTextPlane(scene, "Z", "blue", 128, 128, 60);
...
export function createSceneAxisIndicator(axisContext, rotationQuaternion) {
    ...
    axisContext.stroke();
    createAxisArrow(axisContext, xAxisEnd.x, xAxisEnd.y, 'red');
    ...
    axisContext.stroke();
    createAxisArrow(axisContext, yAxisEnd.x, yAxisEnd.y, 'green');
    ...
    axisContext.stroke();
    createAxisArrow(axisContext, zAxisEnd.x, zAxisEnd.y, 'blue');
    axisContext.fillStyle = 'blue';
    axisContext.fillText('Z', zAxisEnd.x + 5, zAxisEnd.y);
}

function createAxisArrow(context, x, y, color) {
    const headLength = 5;
