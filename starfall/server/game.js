...
export class Game {
    ...
    handleClientMessage(ws, data) {
        if (data.type === 'newShip') {
            ...
            // Générer une position aléatoire entre 0 et 500 du centre
            const distance = Math.random() * 500;
            const angle1 = Math.random() * Math.PI * 2; // Angle autour de l'axe Y
            const angle2 = Math.random() * Math.PI; // Angle par rapport à l'axe XZ
            const position = {
            ...
            const newShip = new Ship(id, position); // Passer la position au constructeur
            this.ships[id] = newShip;
            ws.shipId = id;
            console.log(`✅ New ship created: ${id} at position (${position.x.toFixed(2)}, ${position.y.toFixed(2)}, ${position.z.toFixed(2)})`);
    ...
    async handlePlayerDeath(ship) {
        ...
        const payload = {
            token: ship.token, // Assurez-vous que chaque ship a un token
            score: ship.kills, // Le score correspond au nombre de kills
            game: 'starfall'
        };

        try {
            const response = await fetch('http://localhost:22220/newScore', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
            if (!response.ok) {
                console.error(`❌ Failed to send score for ship ${ship.id}:`, response.statusText);
            } else {
                console.log(`✅ Score sent for ship ${ship.id}`);
            }
        } catch (error) {
            console.error(`❌ Error sending score for ship ${ship.id}:`, error);
        }
    }
